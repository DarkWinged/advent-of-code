...
def play_bigno(entries):
    ...
    print('remaining', len(boards))

    for draw in draws:
        print('drawing', draw)
        # print('remaining', len(boards))
        won, score, truthy_boards, won_boards = update_boards(draw, boards, truthy_boards)
        if won:
            for z in won_boards:
                if len(boards) == 1:
                    return score
    ...
    for z in range(len(boards)):
        ...
        if x is not None and y is not None:
            ...
            if check_win(truthy_boards[z], y, x):
                ...
                won = True
                won_boards.append(z-len(won_boards))

    return won, score, truthy_boards, won_boards
